...
class MyConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upscale is not None and min(x.shape[2:]) * 2 >= 128:
            ...
            w = w.permute(1, 0, 2, 3)
            # probably applying a conv on w would be more efficient. also this
            # quadruples the weight (average)?!
            w = F.pad(w, (1, 1, 1, 1))
            w = (
                w[:, :, 1:, 1:]
                + w[:, :, :-1, 1:]
...
class NoiseLayer(nn.Module):
    """adds noise. noise is per pixel (constant over channels) with per-channel
    weight"""

    def __init__(self, channels, randomize_noise=True, use_random_initial_noise=True):
        super().__init__()
        self.weight = nn.Parameter(torch.zeros(channels))
        self.noise = None
        self.randomize_noise = randomize_noise
        self.use_random_initial_noise = use_random_initial_noise
...
class BlurLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        kernel = self.kernel.expand(x.size(1), -1, -1, -1).contiguous()
        x = F.conv2d(
            x,
            kernel,
            stride=self.stride,
        ...
        if use_noise:
            layers.append(
                ("noise", NoiseLayer(
                    ...
                    use_random_initial_noise=use_random_initial_noise))
            )
